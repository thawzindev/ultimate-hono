...
export const validateQuery = <T extends z.ZodType>(schema: T): MiddlewareHandler => {
  return async (c: Context, next: Next) => {
    try {
      ...
      if (!result.success) {
        logger.debug({ errors: result.error.format() }, 'Query validation failed');
        return c.json(
          {
            message: 'Validation error',
            errors: result.error.format(),
 ...
 */
export const validateParams = <T extends z.ZodType>(schema: T): MiddlewareHandler => {
  return async (c: Context, next: Next) => {
    try {
      ...
      if (!result.success) {
        logger.debug({ errors: result.error.format() }, 'Params validation failed');
        return c.json(
          {
            message: 'Validation error',
            errors: result.error.format(),
